...
public final class IPCClient implements Closeable
{
    ...
    public void close()
    {
        if(pipe == null || getStatus() != PipeStatus.CONNECTED)
            return;
        try {
            pipe.close();
        } catch (IOException e) {
        ...
        readThread = new Thread(() -> {
            try
            {
                ...
                while((p = pipe.read()).getOp() != OpCode.CLOSE)
                {
                    ...
                    String evt = json.has("evt") && !json.isNull("evt") ? json.optString("evt", null) : null;
                    Event event = Event.of(evt);
                    ...
                    if(listener != null && "DISPATCH".equals(json.optString("cmd", null)))
                    {
                        try
                        {
